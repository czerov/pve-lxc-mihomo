...
启用三个订阅的自动优选，并修复 X 一直加载、韩国节点误匹配和 GHCR/容器镜像下载缓慢：
...
该脚本会新增“自动优选”：每 5 分钟直接从全部融合订阅的有效节点中测速选优，地区组改为按需测速，避免嵌套代理组健康检查超时和重复探测。“节点选择”“稳定优选”和“漏网之鱼”会优先使用自动优选，同时保留手动切换。脚本还会让 X 使用 `api.x.com` 做目标站可用性测速、修复短代码 `KR` 误匹配，并让 `ghcr.io` 与 `pkg-containers.githubusercontent.com` 优先直连；直连不可用时自动回退“自动优选”。配置校验或热重载失败时自动恢复。
...
飞牛 fnOS 的位置是“Docker -> 镜像仓库 -> 设置 -> 代理设置”。保存并重启 Docker 后，GHCR 流量会真正绕过 `7890`；只在 Mihomo 面板内把“容器镜像”切到 `DIRECT`，流量仍会经过 Docker HTTP 代理。如果所在网络无法直连 GHCR，请移除这两个排除项，让 Mihomo 自动回退代理。
