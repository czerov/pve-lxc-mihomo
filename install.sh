...
NEXUSBOX_PATCHED_REF="${NEXUSBOX_PATCHED_REF:-a59ce608bb2ba552898f1a536ef8c4a5b940b908}"
...
fetch_url_via_running_proxy() {
  ...
  for proxy in http://127.0.0.1:7890 http://127.0.0.1:7891; do
    if curl -fsSL -x "$proxy" --connect-timeout 5 --max-time 15 --retry 1 -o "$output" "$url"; then
      return 0
    fi
  done
  return 1
...
nexusbox_expected_sha256() {
  case "$1" in
    amd64) echo "443af7d019f92459cb692c86cf0161a7251c240c3a9344831657a3533b6e3408" ;;
    arm64) echo "e1aad8f69667dd1f2c145ef2c93d6d6663fd5b1f8a9bbd28e2914ad190bc707a" ;;
    *) return 1 ;;
  esac
}
...
install_container_image_watchdog() {
  ...
  case "$CONTAINER_WATCHDOG_ENABLE" in
    1|true|yes|on) ;;
  ...
  if ! grep -q '^  - {name: 容器镜像,' "$runtime_config"; then
    say "当前配置没有容器镜像分组，跳过镜像守护服务安装。"
    return 0
  fi

  installer="$WORK_DIR/install-container-image-watchdog.sh"
  raw="https://raw.githubusercontent.com/${NEXUSBOX_PATCHED_REPO}/${NEXUSBOX_PATCHED_BRANCH}/install-container-image-watchdog.sh"
  [ -z "$CONTAINER_WATCHDOG_INSTALLER_URL" ] || urls+=("$CONTAINER_WATCHDOG_INSTALLER_URL")
  urls+=(
    "https://gh-proxy.com/${raw}"
    "https://gh.llkk.cc/${raw}"
    "https://cdn.jsdelivr.net/gh/${NEXUSBOX_PATCHED_REPO}@${NEXUSBOX_PATCHED_BRANCH}/install-container-image-watchdog.sh"
    "$raw"
  )
  for url in "${urls[@]}"; do
  ...
  if [ "$downloaded" != "1" ]; then
    say "警告：镜像守护服务安装器下载失败，核心安装不受影响。"
    return 0
  fi
  if ! bash "$installer"; then
    say "警告：镜像守护服务安装失败，核心安装不受影响。"
    return 0
  fi
